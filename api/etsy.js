...
// cache em memória — persiste enquanto a função Edge está ativa (minutos a horas)
const cache = new Map();
...
export default async function handler(req) {
  ...
  if (cached && Date.now() - cached.ts < CACHE_TTL) {
    return new Response(JSON.stringify({ ...cached.data, _cached: true }), {
      status: 200,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, s-maxage=21600' }
    });
  }
  ...
  try {
    const r = await fetch('https://api.ehunt.ai/api/v1/items', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-VIP-TOKEN': EHUNT_TOKEN },
      body: JSON.stringify({ search_key: q, sort_by: sortBy, desc: 1, page_num: 1, page_size: limit }),
    ...
    const payload = { results, allTags, total: d.data?.product_num || results.length, credits_remaining: d.remaining_today };
