export const config = { runtime: 'edge' };

const CLIENT_ID = '4547631186296426';
const CLIENT_SECRET = 'VOa39QENrlrlQ1auQ8GqILoskKCPutgG';

let tokenData = null;

async function getToken() {
  if (tokenData && Date.now() < tokenData.expiry) return tokenData.token;
  
  const body = new URLSearchParams({
    grant_type: 'client_credentials',
    client_id: CLIENT_ID,
    client_secret: CLIENT_SECRET
  });

  const r = await fetch('https://api.mercadolibre.com/oauth/token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' },
    body: body.toString()
  });

  if (!r.ok) {
    const t = await r.text();
    throw new Error(`Token ${r.status}: ${t.substring(0,100)}`);
  }
  
  const d = await r.json();
  tokenData = { token: d.access_token, expiry: Date.now() + (d.expires_in - 300) * 1000 };
  return tokenData.token;
}

export default async function handler(req) {
  const { searchParams } = new URL(req.url);
  const q = searchParams.get('q');

  if (!q) return new Response(JSON.stringify({ results: [] }), {
    status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
  });

  try {
    const token = await getToken();
    const url = `https://api.mercadolibre.com/sites/MLB/search?q=${encodeURIComponent(q)}&limit=25&sort=sold_quantity_desc`;
    const r = await fetch(url, {
      headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
    });

    if (!r.ok) {
      const text = await r.text();
      return new Response(JSON.stringify({ results: [], _err: `${r.status}: ${text.substring(0,100)}` }), {
        status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
      });
    }

    const data = await r.json();
    return new Response(JSON.stringify(data), {
      status: 200,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, s-maxage=60' }
    });
  } catch(e) {
    return new Response(JSON.stringify({ results: [], _err: e.message }), {
      status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
    });
  }
}
